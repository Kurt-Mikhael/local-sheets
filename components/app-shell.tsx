...
import { localDb } from '@/lib/infrastructure/local/db'

const SpreadsheetEditor = dynamic(() => import('@/components/spreadsheet-editor'), {
  ssr: false,
  loading: () => <div className="editor-loading">Memuat mesin spreadsheet…</div>,
})
...
export default function AppShell() {
  ...
  const syncNow = useCallback(async () => {
    ...
    try {
      ...
      // hanya reload editor kalau versi remote benar-benar diterapkan
  ...
  // seed editor hanya saat ganti workbook aktif
  useEffect(() => {
    if (!active) return
  ...
  // autosave lokal saja; jangan reload editor dari callback ini
