...
import { UNIVER_APP_VERSION } from '../domain/workbook'
...
function toHexRgb(color: { rgb?: string; theme?: number; indexed?: number; auto?: number; tint?: number } | undefined, themeColors: Record<number, string> = {}): string | undefined {
  ...
  if (color.theme !== undefined) {
    ...
    const blend = (c: number) => tint >= 0 ? Math.round(c + (255 - c) * tint) : Math.round(c * (1 + tint))
...
async function parseStylesXlsx(buffer: ArrayBuffer): Promise<{
  ...
  const themeColors = await loadThemeColors(loadedZip)
  const stylesXml = await loadedZip.file('xl/styles.xml')?.async('string')
  if (!stylesXml) return { fonts, fills, borders, cellXfs, numFmts, themeColors }
  ...
  while ((m = fontRe.exec(stylesXml)) !== null) {
    ...
    if (colM) {
      ...
      if (themeM) c.theme = parseInt(themeM[1], 10)
      const indexedM = attrs.match(/indexed="([^"]*)"/)
      if (indexedM) c.indexed = parseInt(indexedM[1], 10)
      const tintM = attrs.match(/tint="([^"]*)"/)
      if (tintM) c.tint = parseFloat(tintM[1])
      if (Object.keys(c).length) font.color = c
    }
    fonts.push(font)
  }
  ...
  return { fonts, fills, borders, cellXfs, numFmts, themeColors }
}
...
async function parseSheetXmlInfo(zip: JSZip, path: string): Promise<{
  path: string
  ref?: XLSX.Range
  ...
  if (borderId > 0 && borderId < borders.length && (xf.applyBorder === '1' || xf.applyBorder === undefined)) {
    ...
    for (const [side, tag] of [['t', 'top'], ['r', 'right'], ['b', 'bottom'], ['l', 'left']] as const) {
      ...
      if (bs?.style) {
        ...
        if (s) {
          const cl = bs.color ? toHexRgb(bs.color, themeColors) : undefined
          bd[side] = { s, cl: cl ? { rgb: cl } : { rgb: '' } }
        }
      }
    }
...
async function parseExcelToSnapshot(file: File, titleHint?: string): Promise<ImportedWorkbook> {
...
interface ImportResponse {
  id: number
  ok: boolean
  result?: { workbookId: string; ownerId: string; title: string; createdBy: string }
  error?: string
}

self.addEventListener('message', async (e: MessageEvent<ImportRequest>) => {
  const { id, buffer, fileName, titleHint } = e.data
  const respond = (msg: ImportResponse) => self.postMessage(msg)
  try {
    const file = new File([buffer], fileName)
    const imported = await parseExcelToSnapshot(file, titleHint)
    const res = await fetch('/api/admin/workbooks/import', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ title: imported.title, snapshot: imported.snapshot }),
    })
    if (!res.ok) {
      const text = await res.text().catch(() => '')
      throw new Error(`HTTP ${res.status}: ${text.slice(0, 200)}`)
    }
    const result = await res.json() as { workbookId: string; ownerId: string; title: string; createdBy: string }
